...
const projectSchema = z.object({
  project_name: z.string().min(1, "프로젝트명을 입력해 주세요."),
  role: z.string().min(1, "담당 역할을 입력해 주세요."),
...
export default function ProjectModal({
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setSubmitError(err.message || "프로젝트 정보를 저장하지 못했습니다.");
    }
  };
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider block mb-1">
                프로젝트명 <span className="text-red-500 font-extrabold">*</span>
              </label>
              <Input
                placeholder="예: 빌링 서비스 개선"
                {...register("project_name")}
                className={`bg-white h-9 border-slate-200 text-xs focus:ring-primary/20 ${
                  errors.project_name ? "border-red-500 focus:ring-red-200" : ""
                }`}
              />
              {errors.project_name && (
            ...
            <div>
              <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider block mb-1">
                담당 역할 <span className="text-red-500 font-extrabold">*</span>
              </label>
              <Input
