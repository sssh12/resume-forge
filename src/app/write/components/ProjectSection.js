    ...
    <div className="space-y-4">
          ...
          isReadOnly ? (
            ...
            >
              ...
              <p className="text-xs text-slate-500 font-bold group-hover:text-slate-900 transition-colors">
                프로젝트 정보(프로젝트명, 역할 등)를 입력해 보세요
              </p>
            </div>
          )
        ) : (
          projects.map((proj) => {
            const isTargetingThis = activeSession?.target_id === proj.id;
            return (
              <div
                key={proj.id}
                ...
                <div className="flex justify-between gap-4">
                  <div className="space-y-1.5 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-extrabold text-base text-slate-900">{proj.project_name}</span>
                      {proj.role && (
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-slate-500 font-semibold">
                      ...
                      {proj.start_date
                        ? new Date(proj.start_date).toLocaleDateString("ko-KR", {
                            year: "numeric",
                            month: "short"
                          })
                        : "미상"}
                      ...
                      !isReadOnly && (
                        <div className="bg-blue-50/35 border border-blue-100/40 rounded-xl p-3.5 mt-2 flex items-center gap-2.5">
                          ...
                          <p className="text-xs text-slate-500 font-semibold">
                            아직 작성된 설명이 없습니다. AI 챗봇을 시작하여 프로젝트 성과를 구조화해 보세요.
                          </p>
                        </div>
                      )
                    )}
                  ...
                  {!isReadOnly && (
                    <div className="flex flex-col items-end gap-2.5 shrink-0">
                      ...
                      <Button
                        size="sm"
                        variant={proj.description ? "outline" : "primary"}
                        onClick={() => onStartChat("projects", proj)}
                      ...
                      >
                        ...
                        {proj.description ? "AI 챗봇 다시 열기" : "AI 챗봇 작성"}
                      </Button>
                    </div>
                  )}
                </div>
      ...
      {!isReadOnly && (
        <ProjectModal
          ...
          onSave={async (id, form) => {
            const success = await onSaveProj(id, form);
            if (success) {
      ...
      {!isReadOnly && (
        <ConfirmModal
          ...
          title="프로젝트 정보 삭제"
          description="정말 이 프로젝트 정보를 삭제하시겠습니까? 관련된 AI 대화도 삭제됩니다."
          confirmText="삭제"
          cancelText="취소"
          onConfirm={async () => {
            setIsDeleteConfirmOpen(false);
            if (deleteTargetId) {
              await onDeleteProj(deleteTargetId);
